...
class Settings_Fields_Site {
	...
	public function settings() {
		...
		add_settings_section(
			...
			sprintf(
				'%1s %2s',
				get_bloginfo( 'name' ),
				__( 'Extended Settings', 'canyon-plugin' )
			),
		...
		add_settings_field(
			...
			__( 'Demo Field', 'canyon-plugin' ),
			[ $this, 'adams_demo_extended_field_callback' ],
			'adams-site-extended',
			'adams-site-extended',
			[ esc_html__( 'Check this box', 'canyon-plugin' ) ]
		);
		...
		add_settings_field(
			...
			__( 'Hide Addon Settings', 'canyon-plugin' ),
			[ $this, 'adams_hide_addon_settings_callback' ],
			CANYON_PARENT_PREFIX . '-site-admin-menu',
			CANYON_PARENT_PREFIX . '-site-admin-menu',
			[ sprintf(
				'%1s %2s %3s',
				esc_html__( 'Hide the', 'canyon-plugin' ),
				CANYON_CHILD_NAME,
				esc_html__( 'link in the admin menu', 'canyon-plugin' )
			) ]
		);
	...
	public function adams_field_section_extended_callback() {
		...
		$html = sprintf(
			...
			esc_html( 'This demo tab was added by the', 'canyon-plugin' ),
			CANYON_PARENT_NAME,
			esc_html( 'plugin.', 'canyon-plugin' )
		);
